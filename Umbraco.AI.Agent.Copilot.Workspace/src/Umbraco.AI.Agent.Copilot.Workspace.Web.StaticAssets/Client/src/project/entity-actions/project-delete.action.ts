 ...
 * (umbConfirmModal resolves them). A rejected delete (e.g. the project still has conversations) is
 * surfaced automatically: `configureAiClient` sets `throwOnError`, so `tryExecute` catches the failure
 * and shows the server's own ProblemDetails as a notification — no local handling needed here.
 */
