...
//
// PORTED FROM v17/release/2026.08.1: this same repro was built and verified there first (against
// CMS 17.5.0, where the scope type is named `IEfCoreScope<T>`, lowercase "f"). CMS 18.0.0 renamed
// it to `IEFCoreScope<T>` — the only change needed to port this file was that rename plus bumping
// `Microsoft.NET.Test.Sdk` to the floor Umbraco.Cms.Tests.Integration 18.0.0 requires (18.5.1). The
// bug, its mechanism, and the fix are identical on both version lines.
//
...
internal sealed class AmbientScopeRaceTests : UmbracoIntegrationTest
{
    ...
    {
        // NOTE: this pinned CMS version (18.0.0) names the scope type `IEFCoreScope<T>` (capital
        // "F") - the v17 port of this same test uses `IEfCoreScope<T>` (lowercase "f") since that
        // is what CMS 17.5.0 shipped. Matched to the restored package.
