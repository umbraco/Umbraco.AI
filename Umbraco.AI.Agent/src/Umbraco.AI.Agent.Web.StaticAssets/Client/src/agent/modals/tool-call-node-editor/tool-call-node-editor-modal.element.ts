...
} from "./tool-call-node-editor-modal.token.js";
import type { UaiOrchestrationNode, UaiToolCallNodeConfig } from "../../types.js";
import { isToolCallNodeConfig } from "../../types.js";
import "@umbraco-ai/core";

/**
 * Modal for editing Tool Call node configuration.
 * Allows selecting a registered AI tool to execute.
 */
@customElement("uai-orchestration-tool-call-node-editor-modal")
export class UaiOrchestrationToolCallNodeEditorModalElement extends UmbModalBaseElement<
	UaiOrchestrationToolCallNodeEditorModalData,
	UaiOrchestrationToolCallNodeEditorModalValue
> {
	@state()
	...
	get #config(): UaiToolCallNodeConfig {
		if (isToolCallNodeConfig(this._node.config)) {
	...
	#onToolChange(event: UmbChangeEvent) {
		...
		const config: UaiToolCallNodeConfig = { ...this.#config, toolId: toolIds[0] ?? null };
				...
				<div id="main">
					<uui-box>
						...
						>
							<uai-tool-picker
								slot="editor"
								.value=${this.#config.toolId ? [this.#config.toolId] : undefined}
...
export default UaiOrchestrationToolCallNodeEditorModalElement;
