...
import {
    ...
    type UaiOrchestrationNodeTypePickerModalValue,
} from "../modals/node-type-picker/node-type-picker-modal.token.js";
import {
...
export class UaiOrchestrationGraphEditorElement extends UmbLitElement {
        ...
        requestAnimationFrame(async () => {
            ...
            try {
                this._editorInstance = await createReteEditor(
                    ...
                    (nodeId, nodeType) => this.#onNodeDoubleClicked(nodeId, nodeType),
                );
    ...
    async #onAddNodeClicked() {
        ...
        const modal = this.#modalManagerContext.open(this, UAI_ORCHESTRATION_NODE_TYPE_PICKER_MODAL, {
            data: {
                nodeTypes: getAddableNodeTypes(),
            },
        });

        const result = await modal.onSubmit().catch(() => undefined) as UaiOrchestrationNodeTypePickerModalValue | undefined;
    ...
    async #openNodeEditorModal(node: UaiOrchestrationNode) {
        ...
        switch (node.type) {
            case "Agent": {
                const modal = this.#modalManagerContext.open(this, UAI_ORCHESTRATION_AGENT_NODE_EDITOR_MODAL, {
                    data: { node },
                });
                const result = await modal.onSubmit().catch(() => undefined);
                if (result?.node) this.#updateNodeInGraph(result.node);
                break;
            }
            case "Function": {
                const modal = this.#modalManagerContext.open(this, UAI_ORCHESTRATION_FUNCTION_NODE_EDITOR_MODAL, {
                    data: { node },
                });
                const result = await modal.onSubmit().catch(() => undefined);
                if (result?.node) this.#updateNodeInGraph(result.node);
            ...
            case "Router": {
                const modal = this.#modalManagerContext.open(this, UAI_ORCHESTRATION_ROUTER_NODE_EDITOR_MODAL, {
                    data: { node },
                });
                const result = await modal.onSubmit().catch(() => undefined);
                if (result?.node) this.#updateNodeInGraph(result.node);
            ...
            case "Aggregator": {
                const modal = this.#modalManagerContext.open(this, UAI_ORCHESTRATION_AGGREGATOR_NODE_EDITOR_MODAL, {
                    data: { node },
                });
                const result = await modal.onSubmit().catch(() => undefined);
                if (result?.node) this.#updateNodeInGraph(result.node);
            ...
            case "Manager": {
                const modal = this.#modalManagerContext.open(this, UAI_ORCHESTRATION_MANAGER_NODE_EDITOR_MODAL, {
                    data: { node },
                });
                const result = await modal.onSubmit().catch(() => undefined);
                if (result?.node) this.#updateNodeInGraph(result.node);
