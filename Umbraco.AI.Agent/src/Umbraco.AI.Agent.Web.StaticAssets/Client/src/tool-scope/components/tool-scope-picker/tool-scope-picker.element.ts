...
>(UmbLitElement, undefined) {
    /**
     * Allow selecting multiple tool scopes.
     */
    ...
    async #openPicker() {
        ...
        const modal = modalManager.open(this, UAI_ITEM_PICKER_MODAL, {
            data: {
                fetchItems: () => this.#fetchAvailableToolScopes(),
                selectionMode: this.multiple ? 'multiple' : 'single',
                title: this.localize.term('uaiAgent_selectToolScope'),
                noResultsMessage: this.localize.term('uaiAgent_noToolScopesAvailable'),
            },
        });
    ...
    #renderAddButton() {
        ...
        return html`
            <uui-button
                ...
                label=${this.localize.term('uaiAgent_addToolScope')}>
