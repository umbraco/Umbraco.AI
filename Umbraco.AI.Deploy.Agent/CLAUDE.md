...
> **Note:** This is the Umbraco.AI.Deploy.Agent package. See the [root CLAUDE.md](../CLAUDE.md) for shared coding standards, build commands, and repository-wide conventions that apply to all packages.

## Build Commands

```bash
# Build the solution
dotnet build Umbraco.AI.Deploy.Agent.sln

# Run tests
dotnet test Umbraco.AI.Deploy.Agent.sln
```

## Architecture Overview

Umbraco.AI.Deploy.Agent provides Umbraco Deploy integration for AI agents. It enables deployment of agents with their tool permissions, user group configurations, and scoping rules.
...
| Project                                 | Purpose                                  |
| --------------------------------------- | ---------------------------------------- |
| `Umbraco.AI.Deploy.Agent`               | Service connector, artifact, notification handlers |
| `Umbraco.AI.Deploy.Agent.Tests.Unit`    | Unit tests                               |
