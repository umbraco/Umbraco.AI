...
public interface IAIConfiguredChatCapability : IAIConfiguredCapability
{
    ...
    /// </summary>
    /// <param name="profileSettings">The resolved, typed profile settings, or <c>null</c> when the profile declares none.</param>
    /// <param name="modelId">Optional model ID to use. If null, the provider's default model is used.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>A configured chat client.</returns>
    /// <remarks>
    ...
    /// </remarks>
    Task<IChatClient> CreateClientAsync(object? profileSettings, string? modelId, CancellationToken cancellationToken)
