...
describe("UaiEntityRequestContextContributor", () => {
    const TEST_HOST_TAG = "uai-test-controller-host";
     ...
     * than being mocked away. Appending to a connected document fires `connectedCallback`, which
     * `UmbControllerHostElementMixin` wires to `hostConnected()` -- a real `UmbLitElement` gets this
     * for free from its own custom element lifecycle; this test element needs the same registration
     * to activate its hosted controllers (their context-request listeners never attach otherwise).
     */
