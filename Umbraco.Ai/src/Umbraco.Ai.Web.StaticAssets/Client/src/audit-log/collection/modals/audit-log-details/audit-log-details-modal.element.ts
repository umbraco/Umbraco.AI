        ...
        return html`
            <umb-body-layout>
                <div slot="header" class="header-layout">
                    <div>
                        <!-- Prev Button -->
                        <uui-button look="secondary" compact @click=${this.#onPrevClick} ?disabled=${this.data!.uniques!.indexOf(this.value!.unique!) === 0}>
                            <uui-icon name="icon-arrow-left"></uui-icon>
                        </uui-button>
                    </div>
                    <div>
                        <h3>Audit Log Details <small>[${ (this.data?.uniques?.indexOf(this.value?.unique ?? this.data?.uniques[0]) ?? 0) + 1 }/${this.data?.uniques?.length ?? 0}]</small></h3>
                        <small><strong>ID:</strong> ${this._auditLog?.unique}</small>
                    </div>
                    <div>
                        <!-- Next Button -->
                        <uui-button look="secondary" compact @click=${this.#onNextClick} ?disabled=${this.data!.uniques!.indexOf(this.value!.unique!) === this.data!.uniques!.length - 1}>
                        ...
                        : html`
                            <table>
                                <tr>
                                    <th>Timestamp</th>
                                    <td>
                                        <div>${this._auditLog?.startTime ? this.#formatTimestamp(this._auditLog?.startTime) : "—"}</div>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Duration</th>
                                    <td>
                                        <div>${this._auditLog?.durationMs ? `${(this._auditLog?.durationMs / 1000).toFixed(2)}s` : "—"}</div>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Status</th>
                                ...
                                <tr>
                                    <th>User</th>
                                    <td>
                                        <div>${this._auditLog?.userName ?? this._auditLog?.userId ?? "Anonymous"}</div>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Feature</th>
                                    <td>
                                        ...
                                        <small>
                                            ${this._auditLog?.featureId}${this._auditLog?.featureVersion ? html`/v${this._auditLog?.featureVersion}` : ''}
                                        </small>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            ${this._auditLog?.profileAlias}
                                        </div>
                                        <small>
                                            ${this._auditLog?.profileId}${this._auditLog?.profileVersion ? html`/v${this._auditLog?.profileVersion}` : ''}
                                        </small>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <th>Tokens</th>
                                    <td>
                                        <div>${this._auditLog?.inputTokens ?? 0} / ${this._auditLog?.outputTokens ?? 0}</div>
                                    </td>
                                </tr>
                                ${this._auditLog?.errorMessage ? html`
                                    <tr>
                                        <th>Error Message</th>
                                        <td>
                                ...
                                ` : ''}
                                ${this._auditLog?.promptSnapshot ? html`
                                    <tr>
                                        <th>Prompt</th>
                                        <td>
                                ...
                                ` : ''}
                                ${this._auditLog?.responseSnapshot ? html`
                                    <tr>
                                        <th>Response</th>
                                        <td>
