...
public class VersionSnapshotContextController : ContextControllerBase
{
    ...
    [HttpGet($"{{{nameof(contextIdOrAlias)}}}/versions/{{{nameof(version)}:int}}")]
    [MapToApiVersion("1.0")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var snapshot = await _contextService.GetContextVersionSnapshotAsync(context.Id, version, cancellationToken);
        if (snapshot is null)
        {
