# ============================================================================
# Umbraco.Ai Monorepo Change Detection Script (GitHub Actions)
# ============================================================================
...
param(
    [string]$SourceBranch = $env:GITHUB_REF,
    [string]$SourceVersion = $env:GITHUB_SHA,
...
# ============================================================================
# OUTPUT GENERATION (GITHUB ACTIONS FORMAT)
# ============================================================================

function Write-GitHubActionsOutputs {
    <#
    .SYNOPSIS
    Outputs GitHub Actions outputs for changed products and build levels.
    Uses $GITHUB_OUTPUT file for output variables.
    #>
    param(
        [hashtable]$Products,
        [hashtable]$ChangedProducts,
    ...
    # Create JSON structure for dynamic matrix generation (GitHub Actions format with "include")
    for ($level = 0; $level -le $maxLevel; $level++) {
        ...
        foreach ($productKey in $Products.Keys) {
            if ($BuildLevels[$productKey] -eq $level) {
                ...
                # If changed, add to matrix array
        ...
        if ($productsAtLevel.Count -gt 0) {
            ...
            # Set level variables (kebab-case for GitHub Actions)
            $anyChanged = ($changedAtLevel.Count -gt 0).ToString().ToLower()

            # Write to GITHUB_OUTPUT file
            Add-Content -Path $env:GITHUB_OUTPUT -Value "level$level-changed=$anyChanged"

            # Output matrix JSON for this level (GitHub Actions "include" format)
            if ($matrixArray.Count -gt 0) {
                # GitHub Actions matrix format requires "include" key
                $matrixObject = @{ include = $matrixArray }
                $matrixJsonStr = $matrixObject | ConvertTo-Json -Depth 3 -Compress

                Add-Content -Path $env:GITHUB_OUTPUT -Value "level$level-matrix=$matrixJsonStr"
                Write-Host "    Matrix JSON: $matrixJsonStr" -ForegroundColor Magenta
            }
            else {
                # Empty matrix - still output valid JSON
                $emptyMatrix = @{ include = @() }
                $emptyMatrixJson = $emptyMatrix | ConvertTo-Json -Compress
                Add-Content -Path $env:GITHUB_OUTPUT -Value "level$level-matrix=$emptyMatrixJson"
            }
        }
    }
}
...
Write-Host "==================================="
Write-Host "Umbraco.Ai Change Detection (GH)"
Write-Host "==================================="
Write-Host ""
...
# 6. Output GitHub Actions outputs
Write-GitHubActionsOutputs -Products $products -ChangedProducts $changedProducts -BuildLevels $buildLevels -RootPath $RootPath
