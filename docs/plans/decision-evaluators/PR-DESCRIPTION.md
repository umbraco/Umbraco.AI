...
- Public API: three Management API controllers gain an `IAIExperimentalFeatures` constructor parameter. The old constructors are kept `[Obsolete("... Will be removed in v19.")]` and the new ones are `[ActivatorUtilitiesConstructor]`. No route, DTO, or OpenAPI change, so no client regeneration.
