...
> **Line status:** on `v17/dev`, only the interrupt/resume plumbing this brief depends on has
> shipped — the backend content/media write tools (PRs #329/#330 on `v18/dev`) haven't been
> backported here yet, and the Automate-side wiring this brief describes hasn't been independently
> verified either. See `PLAN.md` in this folder.
