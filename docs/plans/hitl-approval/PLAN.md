...
> **Status:** Partially shipped on this line. The generic interrupt/resume plumbing exists
> (`AGUIInterruptKind` + tests, commit `9dd53a72`), but the concrete backend content/media write
> tools added on `v18/dev` via PRs #329/#330 are not ancestors of `v17/dev` — they haven't been
> backported yet. Don't treat this as done here the way it is on v18.
