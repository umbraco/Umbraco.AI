...
Workflows can declare typed settings that are configurable in the backoffice. Settings are defined as a C# class with `[AIField]` attributes for UI generation.
...
The interface that all workflows implement:
...
| `SettingsType`      | Settings type (null if no settings)                      |
| `GetSettingsSchema` | Returns the dynamic form schema for settings             |
| `BuildWorkflowAsync`| Builds the MAF workflow from agent definition and settings |

### AIAgentWorkflowBase

Base class for workflows **without** settings. Reads `Id`, `Name`, and `Description` from the `[AIAgentWorkflow]` attribute automatically.

### AIAgentWorkflowBase\<TSettings>

Base class for workflows **with** typed settings. Handles JSON deserialization of settings and generates a backoffice form schema from `[AIField]` attributes.

### AIAgentWorkflowAttribute

Marker attribute for auto-discovery:

```csharp
[AIAgentWorkflow("my-workflow-id", "My Workflow Name", Description = "Optional description")]
```

## Workflow Patterns

The MAF `AgentWorkflowBuilder` provides built-in patterns:
...
```csharp
return AgentWorkflowBuilder.BuildSequential("pipeline", [agentA, agentB, agentC]);
// Flow: agentA → agentB → agentC
```

### Custom Patterns

For more complex patterns, build the `Workflow` directly using the MAF API. Refer to the Microsoft Agents Framework documentation for advanced workflow construction.

## Deployment

Orchestrated agents are deployed via [Umbraco.AI.Agent.Deploy](../deploy/README.md) like any other agent. The deployment artifact includes:

- Agent type (`Standard` or `Orchestrated`)
- Type-specific configuration (workflow ID and settings for orchestrated agents)
- Profile reference

{% hint style="warning" %}
The workflow implementation must be registered in the target environment. The deployment only transfers the agent configuration, not the workflow code.
{% endhint %}
