...
The search add-on registers an index called `UmbAI_Search` with the CMS Search framework. This means it receives the same content change notifications as other indexes such as Examine.
...
The agent tool runs in the backoffice and respects the current user's start node restrictions. Users with restricted content or media start nodes only see results under their allowed tree branches. If the content cannot be resolved to verify the path, it is excluded when restrictions are in place.
