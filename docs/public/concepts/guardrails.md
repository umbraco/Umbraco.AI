...
A guardrail is a collection of rules that evaluate AI content at runtime to prevent unsafe, non-compliant, or low-quality responses. Guardrails support both **pre-generate** (input filtering) and **post-generate** (response validation) phases, enabling comprehensive control over what goes into and comes out of AI operations.
...
| `Action`      | What happens when flagged: `Block` or `Warn`         |
...
1. **Safety** - Prevent toxic, harmful, or inappropriate content from reaching users
2. **Compliance** - Enforce data protection by detecting PII in inputs and outputs
3. **Quality** - Use LLM-as-judge evaluation to ensure responses meet brand standards
4. **Flexibility** - Combine multiple evaluators with different phases and actions
5. **Reusability** - Assign the same guardrail to multiple profiles, prompts, and agents
...
| `data-protection`    | GDPR compliance         | PII pre-generate (Block), PII post (Block)|
...
| Evaluator    | ID          | Type       | Description                                                       |
| ------------ | ----------- | ---------- | ----------------------------------------------------------------- |
| PII          | `pii`       | Code-based | Regex patterns for emails, phone numbers, SSNs, credit cards      |
| Toxicity     | `toxicity`  | Code-based | Keyword and pattern matching with configurable word lists          |
| LLM Judge    | `llm-judge` | Model-based| Uses an AI model to evaluate content against configurable criteria |
...
4. The request proceeds to the AI provider
5. **Post-generate** rules evaluate the response
6. If any post-generate rule with `Block` action flags content, an `AIGuardrailBlockedException` is thrown
7. Rules with `Warn` action attach metadata but allow content through
