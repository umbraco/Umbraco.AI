...
import type { UAiAgentRegistrationModel, UAiAgentPropertyActionMeta } from './types.js';
import type { UAiAgentScopeConditionConfig } from './prompt-scope.condition.js';
 ...
 */
function getPropertyEditorUisForScope(prompt: UAiAgentRegistrationModel): string[] {
...
export function generatePromptPropertyActionManifest(
    prompt: UAiAgentRegistrationModel,
    weight: number = 100
): ManifestPropertyAction<UAiAgentPropertyActionMeta> | null {
    ...
    return {
        ...
        meta: {
            ...
            Agentscope: prompt.scope,
        },
        conditions: [
            {
                alias: UAI_PROMPT_SCOPE_CONDITION_ALIAS,
                scope: prompt.scope,
            } as UAiAgentScopeConditionConfig,
        ],
    };
}
