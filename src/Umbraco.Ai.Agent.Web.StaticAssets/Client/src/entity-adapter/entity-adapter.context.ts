 ...
 *
 * Provides entity detection and serialization for the Copilot sidebar.
...
import { UaiDocumentAdapter } from "./adapters/document.adapter.js";
import type {
	UaiDetectedEntity,
	UaiEntityAdapterApi,
	UaiSerializedEntity,
 ...
 * - Match workspaces to entity adapters
...
export class UaiEntityAdapterContext {
	/**
	 * Available entity adapters.
	 * For minimal implementation, only document adapter is registered.
	 * Future: Load from extension registry.
	 */
	readonly #adapters: UaiEntityAdapterApi[] = [new UaiDocumentAdapter()];
	...
	#refresh(): void {
		...
		for (const entry of entries) {
			...
			const adapter = this.#adapters.find((a) => a.canHandle(entry.context));
