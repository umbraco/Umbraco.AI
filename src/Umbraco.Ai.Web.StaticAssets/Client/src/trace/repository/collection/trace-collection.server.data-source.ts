...
import type { UaiTraceItemModel } from "../../types.js";

/**
 * Server data source for Trace collection operations.
 */
export class UaiTraceCollectionServerDataSource implements UmbCollectionDataSource<UaiTraceItemModel> {
    ...
    async getCollection(_filter: UmbCollectionFilterModel) {
        ...
        const items = data.items.map(UaiTraceTypeMapper.toItemModel);
        ...
        */
        ...
        console.warn('TraceService not yet available - using mock data');
        return {
            data: {
                items: [],
                total: 0,
