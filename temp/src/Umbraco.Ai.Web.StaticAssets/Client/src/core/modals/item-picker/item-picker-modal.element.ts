...
> {
    ...
    render() {
		...
		return html`
			<umb-body-layout headline=${this.data?.title ? this.localize.string(this.data.title) : this.localize.term("ucPlaceholders_selectAnItem")}>
                <div id="main">
                    <uui-input label="Search" type="search" placeholder=${this.localize.term('placeholders_filter')} @input=${this.#filter}>
						<div class="icon-holder" slot="prepend">
							<uui-icon name="search"></uui-icon>
						</div>
					</uui-input>
                    <uui-box>
                        ${when(this._filteredItems.length > 0,
                            () => html`
                                <uui-ref-list>
                                    ${repeat(
                                        this._filteredItems,
                            ...
                            () => when(this._loaded,() => html`<p style="font-size: 14px; text-align: center; margin: var(--uui-size-space-6);">${unsafeHTML(this.localize.string(this.data!.noResultsMessage ?? '#uaiMessage_noResults'))}</p>`))}
                    </uui-box>
                </div>
				<uui-button slot="actions" id="close" label=${this.localize.term("general_close")} @click="${this._rejectModal}"></uui-button>
                ${when(this.data.selectionMode === 'multiple', () => html`<uui-button
					slot="actions"
					id="confirm"
					color="positive"
					look="primary"
